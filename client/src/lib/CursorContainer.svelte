<script>
  ...
  import CursorTemplate from './CursorTemplate.svelte';

  /* Bindings */
  let cursorsContainer;

  /* Set up LiveBlocks cursors */
  onMount(() => {
    const client = createClient({
      authEndpoint: "http://localhost:8080/auth",
    });
    const room = client.enter("musical-cursors-room", { cursor: null });
    /**
...
<CursorTemplate />
<section bind:this={cursorsContainer}></section>